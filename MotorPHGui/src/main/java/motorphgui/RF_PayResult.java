 ...
 * @author admin
 */
public class RF_PayResult extends javax.swing.JDialog {
    
    private final String empID;
    private final String cutoffPeriod;    

    public RF_PayResult(java.awt.Frame parent, boolean modal, String empID, String cutoff) {
        super(parent, modal);
        this.empID = empID;
    ...
    private void processPayroll() {
        ...
        DateTimeFormatter cutoffFormatter = DateTimeFormatter.ofPattern("MMMM d yyyy");
        DateTimeFormatter timeFormatter = DateTimeFormatter.ofPattern("H:mm");

        LocalDate cutoffDate;
        LocalDate startDate;
        try {
            cutoffDate = LocalDate.parse(cutoffPeriod, cutoffFormatter);

            if (cutoffDate.getDayOfMonth() == 15) {
                // Cut-off on 15th covers 1st to 15th
                startDate = cutoffDate.withDayOfMonth(1);
            } else {
                // End of month cut-off covers 16th to last day of month
        ...
        for (int i = 1; i < attendanceData.size(); i++) {
            ...
            try {
                ...
                if (date.isBefore(startDate) || date.isAfter(cutoffDate)) continue;
    ...
    private void btn_Results_HomeActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_Results_HomeActionPerformed
        this.dispose();
        MainDialog_RF mainDialog = new MainDialog_RF(
...
private void exportPayrollToPDF() {
    ...
    public static void main(String args[]) {
        try {
            ...
            java.util.logging.Logger.getLogger(RF_PayResult.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        }

        java.awt.EventQueue.invokeLater(() -> {
            RF_PayResult dialog = new RF_PayResult(new JFrame(), true, "10001", "July 15 2024");
